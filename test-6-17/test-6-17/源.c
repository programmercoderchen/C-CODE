...
void main()
{
	...
	do
	{
		...
		case 8:
			...
			printf("所在的位置是：%d", arrayPossition);
			system("pause"); //程序暂停
			break;
		case 9:
			//添加成绩
...
int binSearchScore(int array[], int len, int searchNum)
{
	...
	int right = (sizeof(array) / sizeof(array[0])) - 1;//要注意防止right下标越界
	while (left <= right)
	{
		mid = (left + right) / 2;
...
int deleteScore(int array[], int length, int deletePossition)
{
	...
	{
		printf("要删除的位置越界，无法删除");
	}
	else
	{
		char c;
		printf("%d\n", array[deletePossition - 1]);
		printf("是否确定要删除以上成绩（Y/N）：\n");
		scanf("%c", &c);
		if (c == 'Y' || c == 'y')
		{
			for (int i = deletePossition-1; i < length; i++)
				array[i] = array[i + 1];//数据的覆盖
			length--;
		}
	}
	return SUCCESS;
